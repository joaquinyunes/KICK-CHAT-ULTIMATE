import { getServerUrl, getAuthHeaders, showMsg, initCommon } from './admin-common.js';
...
async function handleCreateClient() {
  ...
  try {
    const res = await fetch(`${getServerUrl()}/api/admin/users`, { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ username, password, link_url: linkUrl, expires_at: expiresAt }) });
...
async function saveClientEdit(userId) {
  ...
  try {
    await fetch(`${getServerUrl()}/api/admin/users/${userId}`, { method: 'PUT', headers: getAuthHeaders(), body: JSON.stringify({ link_url: linkUrl, expires_at: expiresAt, is_active: isActive }) });
    loadClientes();
    ...
    container.innerHTML = users.map(u => {
      ...
      return `<div class="cliente-card ${vencido ? 'cliente-vencido' : ''}" data-user-id="${u.id}">
        <div class="cliente-header">
          <div><strong>${u.username}</strong><span class="cliente-meta"> Creado: ${fecha}</span></div>
        ...
        <div class="cliente-bots">
          ...
          ${(u.bots || []).length === 0 ? '<span class="cliente-no-bots">Sin bots</span>' :
            (u.bots || []).map(b => `<span class="cliente-bot-tag">${b.bot_name} <button class="btn-unassign" data-bot-id="${b.id}" data-username="${u.username}" title="Quitar bot">X</button></span>`).join('')}
        </div>
